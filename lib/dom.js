...
/**
 * calendar 오늘 날짜 기준으로 일주일 배열을 반환하는 함수
 * @param {void}
 * @returns {array}
 *
 */
...
const createWeekArray = () => {
  ...
  today = Number(now.toLocaleDateString("ko-KR").slice(9, 11));
  for (let i = 0; i < 7; i++) {
    if (i < 3) {
      weekArray.push(today - i);
    }
    if (i > 3) {
      weekArray.push(today + (i - 3));
    }
  }
  weekArray = weekArray.sort((a, b) => a - b);

  return weekArray;
};

/**
 * calendar 일주일 날짜 배열을 요일과 맵핑하여 객체로 반환하는 함수
 * @param {array}
 * @returns {object}
 *
 */

const createWeekObject = (weekArray) => {
  const now = new Date();
  const year = now.getFullYear();
  const month = now.getMonth();

  const dayOfWeek = ["S", "M", "T", "W", "T", "F", "S"];
  return weekArray.map((date) => {
    const obj = new Date(year, month, date);
    const idx = obj.getDay();
    return {
      date,
      day: dayOfWeek[idx],
    };
...
const createCalenderTags = (date, day) => {
  const li = document.createElement("li");
...
const renderCalender = () => {
  ...
  const weekObject = createWeekObject(weekArray);

  const ul = document.querySelector(".calendar");

  ul.innerHTML = "";
  weekObject.map((item) => {
    const li = createCalenderTags(item.date, item.day);
    ul.appendChild(li);
  });
};

/**
 * calendar에서 누른 날짜와 todoList의 date를 비교하여 값을 반환 함수
 * @param {number} clickedDate
...
const compareTodoDate = (clickedDate, toDoList) => {
  const clickedTodoList = toDoList.filter(
    (item) => clickedDate === Number(item.date)
  );
  return clickedTodoList;
};

/**
 * clickedTodoList가 빈 배열일때 todoList대신 보여주는 함수
 * @param
 * @returns
 *
 */

const withoutToDoList = () => {
  alert("등록된 todo가 없습니다!");
};

/**
 *  calendar가 클릭되었을 때, css 변경하는 함수
 * @param {void}
 * @returns {void}
 *
 */
