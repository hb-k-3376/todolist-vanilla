...
/**
 * 오늘을 기준으로 7일 캘린더를 생성합니다
 * @param {array} todoList
 * @returns {void}
 */
const createCalendar = (toDoList) => {
  ...
  for (let i = -3; i <= 3; i++) {
    ...
    // 해당 날짜의 todo 개수
    const count = toDoList.filter(todo => todo.todoDate === dateStr).length;

    const li = document.createElement('li');
    li.className = `calender-item ${i === 0 ? 'active' : ''}`;
    li.setAttribute('data-date', dateStr);

    const dayNames = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];
    li.innerHTML = `
      <p class="day">${dayNames[date.getDay()]}</p>
      <p>${date.getDate()}</p>
      ${count > 0 ? `<span class="todo-count">${count}</span>` : ''}
    `;

    li.addEventListener('click', () => handleDateClick(dateStr));
...
const handleDateClick = (dateStr) => {
  ...
  createCalendar(toDoList);

  // 선택된 날짜의 todo만 필터
  const filtered = toDoList.filter(todo => todo.date === selectedDate); 
  renderList(filtered);
  calcProgress(filtered);
};
